...
export default function FilmCard({film, direction="column", description=false}: FilmCardType) {
  ...
  return (
    <div className={styles[`movie_${direction}`]}>
      <div className={`${styles['movie-info']}`}>
        ...
        <h3 className={`${styles['movie-info__title']}`}>{`${film.name} (${getAgeRating(film.ageRating)})`}</h3>
        ...
        {description && <p className={`${styles.movie__description}`}>{filmDescription(film?.description)}</p>}
      </div>
    </div>
  )
}